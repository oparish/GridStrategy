...
public class Rule
{
	...
	public Rule(List<Integer> integers, boolean player1, RuleHeader ruleHeader)
	{
		...
		int counter = conditionHeader.getSize();
		for (ActionHeader actionHeader : actionHeaders)
		{
			Action action = Action.makeAction(integers.subList(counter, actionHeader.getSize() + counter), 
					player1, actionHeader);
			this.actions.add(action);
			counter += actionHeader.getSize();
		}
	}
	...
	public ArrayList<Byte> getHeaderBytes()
	{
		...
		headerByteList.add(Integer.valueOf(this.actions.size()).byteValue());
		for (Action action : this.actions)
		{
